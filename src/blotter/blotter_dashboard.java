 ...
 * @author Raven
 */
public class blotter_dashboard extends javax.swing.JFrame {

    /**
     * Creates new form blotter_dashboard
     */
    public blotter_dashboard() {
        initComponents();
        displayData();
    }
    public void displayData(){
        try{
            ...
            ResultSet rs = conf.getData("SELECT b_id, c_id, reason, creation_date, status FROM blotter_reports");
    ...
    private void initComponents() {
        ...
        panelprinting = new javax.swing.JLabel();
        ...
        panelprinting.setBackground(new java.awt.Color(153, 255, 255));
        panelprinting.setFont(new java.awt.Font("Playbill", 0, 36)); // NOI18N
        panelprinting.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        panelprinting.setText("Print");
        panelprinting.setOpaque(true);
        panelprinting.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                panelprintingMouseClicked(evt);
            }
            public void mouseEntered(java.awt.event.MouseEvent evt) {
                panelprintingMouseEntered(evt);
            }
            public void mouseExited(java.awt.event.MouseEvent evt) {
                panelprintingMouseExited(evt);
            }
        });
        jPanel3.add(panelprinting, new org.netbeans.lib.awtextra.AbsoluteConstraints(5, 320, 210, 30));
    ...
    private void AddMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_AddMouseClicked
        ...
        if(!Add.isEnabled()){
            ...
            blotter_form bf = new blotter_form();
            if(Uidentifier.getText().equals("Admin")){
                bf.identify("Admin");
            }else{
                bf.identify("Staff");
            }
            bf.cdate.setEnabled(false);
            bf.b_id.setEnabled(false);
            bf.title.setText("Make a Blotter Report");
            bf.enter.setText("Add");
            bf.setVisible(true);
            this.dispose();
        }
        ...
        if (!Edit.isEnabled()) {
            ...
            if (rowIndex < 0) {
                ...
                try {
                    ...
                    ResultSet rs = conf.getData("SELECT * FROM blotter_reports WHERE b_id = '" + tbl.getValueAt(rowIndex, 0) + "'");

                    if (rs.next()) {
                        if (rs.getString("status").equalsIgnoreCase("Finished")) {
                            JOptionPane.showMessageDialog(this, "Cannot Edit Finished Status!");
                            return;
                        }
                        bf.users_table.setEnabled(false);
                        bf.cdate.setEnabled(false);
                        bf.b_id.setEnabled(false);
                        bf.reason.setText(rs.getString("reason"));
                        bf.Statement.setText(rs.getString("statement"));
                        bf.b_id.setText("" + rs.getInt("b_id"));
                        bf.status.setSelectedItem(rs.getString("status"));
                        bf.enter.setText("Update");
                        bf.setVisible(true);
                        this.dispose();
                    } else {
        ...
        if (!Delete.isEnabled()) {
            ...
            if (rowIndex < 0) {
                ...
                try {
                    ...
                    TableModel tbl = users_table.getModel();
                    String b_id = tbl.getValueAt(rowIndex, 0).toString();

                    String sql = "SELECT image FROM blotter_reports WHERE b_id = '" + b_id + "'";
                    ...
                    int confirm = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete this report?", "Confirm Delete", JOptionPane.YES_NO_OPTION);
                    if (confirm == JOptionPane.YES_OPTION) {
                        conf.deleteData("DELETE FROM blotter_reports WHERE b_id = '" + b_id + "'");
                        File imageFile = new File(imgPath);
                        if (imageFile.exists()) {
                            imageFile.delete();
                        }

                        String action = "Deleted blotter with ID No. " + b_id;
    ...
    private void panelprintingMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_panelprintingMouseClicked
        // TODO add your handling code here:
        int rowIndex = users_table.getSelectedRow();
        if (rowIndex < 0) {
            JOptionPane.showMessageDialog(null, "Please select an item!");
        } else {
            PanelPrinting bf = new PanelPrinting();
            try {
                config conf = new config();
                TableModel tbl = users_table.getModel();
                String b_id = tbl.getValueAt(rowIndex, 0).toString();

                ResultSet rs = conf.getData("SELECT * FROM blotter_reports WHERE b_id = '" + b_id + "'");
                if (rs.next()) {
                    String c_id = rs.getString("c_id");

                    ResultSet rs2 = conf.getData("SELECT * FROM citizen WHERE c_id = '" + c_id + "'");
                    if (rs2.next()) {
                        bf.cdate.setText(rs.getString("creation_date"));
                        bf.rname.setText(rs2.getString("lname") + ", " + rs2.getString("fname"));
                        bf.case2.setText(rs.getString("reason"));
                        String statementText = rs.getString("statement");
                        bf.text.setText("<html><body style='width: 300px;'>" + statementText + "</body></html>");
                        if (Uidentifier.getText().equals("Admin")) {
                            bf.identify("Admin");
                        } else {
                            bf.identify("Staff");
                        ...
                        JOptionPane.showMessageDialog(this, "Citizen record not found.");
                    }
                } else {
                    JOptionPane.showMessageDialog(this, "Blotter record not found.");
                }
            } catch (SQLException ex) {
                System.out.println("SQL Error: " + ex.getMessage());
            }
    ...
    }//GEN-LAST:event_panelprintingMouseClicked

    private void panelprintingMouseEntered(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_panelprintingMouseEntered
        // TODO add your handling code here:
        panelprinting.setForeground(Color.white);
        panelprinting.setBackground(Color.gray);
    }//GEN-LAST:event_panelprintingMouseEntered

    private void panelprintingMouseExited(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_panelprintingMouseExited
        // TODO add your handling code here:
        panelprinting.setForeground(Color.black);
        panelprinting.setBackground(null);
    }//GEN-LAST:event_panelprintingMouseExited
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(blotter_dashboard.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new blotter_dashboard().setVisible(true);
            }
        });
    }
    ...
    public javax.swing.JLabel panelprinting;
